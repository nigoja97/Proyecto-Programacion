...
tabla_pruebas <- Pruebas %>% 
  mmtable(table_data = Resultados, table_name = "Prueba AIC/BIC")+
  header_top(Modelo)+
  header_left(Criterio)+
  header_format(Modelo, list(cell_text(transform = "capitalize")))+
  header_format(Criterio,  list(cell_text(transform = "uppercase")))+
  table_format(
    locations = list(
      cells_body(rows = c(2,3), columns = 4)
    ),
    style     = list(
...
tabla_pruebas <- tabla_pruebas %>%
  tab_header(
    title = "Akaike Information Criterion Test",
  )
